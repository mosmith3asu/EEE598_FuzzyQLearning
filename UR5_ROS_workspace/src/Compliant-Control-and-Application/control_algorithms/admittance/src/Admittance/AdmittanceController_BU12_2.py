...
class Admittance(object):
	def __init__(self,dtau = None):
		...
		if SETTINGS['wait4RL']: self.wait_for_ext_reset()
		else: self.reset()
	...
	def compute_admittance(self):
		...
		dt = max(dt,0.7*self.dt)
		dt = min(dt,1.3*self.dt)
		...
		# FILTER VEL ------------------------------------------------------ 
		self.arm_desired_twist_adm_[0] = self.filters['vel'].sample(self.arm_desired_twist_adm_[0])
		
		# Calculate derivatives ---------------------------------------------
		dFdt = (self.state['F'] - wrench_external_[0])/dt #self.dt
		jerk = (self.state['acc'] - arm_desired_accelaration[0])/dt #self.dt
		#
		# acc = arm_desired_accelaration[0]
		# vel = self.arm_desired_twist_adm_[0]
		# pos = self.state['pos'] - arm_desired_accelaration[0] / dt
		#
		...
		self.state['dFdt'] = self.filters['dFdt'].sample(dFdt)
		self.state['jerk'] = self.filters['jerk'].sample(jerk)
	...
	def check_terminal(self): # Check at terminal state
		...
		is_near_goal = ( abs(self.state['pos'] - self.start_pos) > 0.8)
