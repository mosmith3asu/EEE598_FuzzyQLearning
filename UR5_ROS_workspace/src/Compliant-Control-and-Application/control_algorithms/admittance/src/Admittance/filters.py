...
class LowPass_Filter(object):
	...
	def __init__(self,fs,fc,enable=True,mem=True):
		self.fs = fs
		self.dt = 1/fs
		w0 = fc
		...
		self.memory = {'raw': [], 'filt': [],'enable':mem}
		self.enable = enable
	
	
	def save_mem(self,fname):
		if self.memory['enable']:
			if not '.np' in fname: fname + '.np'
			np.save(fname,np.array([self.memory['raw'],self.memory['filt']]))
			print(f'Saved filter memory buffer as [{fname}]...')
	...
	def sample(self,yraw):
		if self.last_yraw is None or self.last_yfilt is None: 
			...
			return 0 if self.enable else yraw
		else:			
			...
			if self.memory['enable']:
				self.memory['raw'].append(self.last_yraw )
				self.memory['filt'].append(self.last_yfilt)
			return yfilt if self.enable else yraw
			
			
	def compare_raw2filt(self,ax2,name='',t=None):
		if self.memory['enable']:
			...
			ax2[0].set_xlim([min(t),max(t)]);
			ax2[1].set_ylabel('y: ' + name) #ax2[0].set_ylabel("$y(t)$")
			#ax2[1].set_title('Filter Result:' + name)
			# Generate Fourier transform
			Yhat = np.fft.fft(Yraw);
			Yfilthat = np.fft.fft(Yfilt)
			fcycles = np.fft.fftfreq(len(t),d=1.0/self.fs )
			...
			#plt.figure()
			ax2[1].plot(fcycles,np.absolute(Yhat));
			ax2[1].plot(fcycles,np.absolute(Yfilthat));
			ax2[1].set_xlim([-100,100]);
			ax2[1].set_xlabel("$\omega$ (cycles/s)");
			#ax2[1].set_ylabel("$|\hat{y}|$");
			ax2[1].set_ylabel('Spectrum ' + name)
			#ax2[1].set_title('Spectrum Analysis' + name)
			#plt.show()
...
def test_main():
	# Generate a signal
	samplingFreq = 80; # sampled at 1 kHz = 1000 samples / second
	fc = 5
	tlims = [0,1]        # in seconds
	signalFreq = [2,50]; # Cycles / second
	signalMag = [1,0.2]; # magnitude of each sine
	...
	#filt = LowPass_Filter(fs = samplingFreq,fc = 2*np.pi*fc )
	filt = LowPass_Filter(fs = samplingFreq,fc = 50 )
