...
class Admittance(object):
    ...
    def wait_for_transformations(self, reset=False):
        ...
        self.state['vel'] = self.current_linear_velocity[0]#self.arm_desired_twist_adm_[0]
        self.state['acc'] = 0
        self.state['jerk'] = 0
    ...
    def check_terminal(self):  # Check at terminal state
        ...
        is_near_goal = (abs(self.current_position[0] - self.start_pos) > 0.2)
        within_var_tol = [
            (abs(self.arm_desired_twist_adm_[0]) < 0.001),
            (abs(self.state['vel']) < 0.001),
            (abs(self.state['acc']) < 0.001),
            (abs(self.state['F']) < 0.001),
            (abs(self.state['dFdt']) < 0.001) ]
    ...
    def report(self):
        ...
        try:
            ...
            print(
                f'\t| pos  : {round(self.start_pos, 2)} => [{round(current_pos,2)}/{round(self.state["pos"], 2)}] => {self.goal_pos}')  # \t {np.round(np.max(self.hist["pos"]),2)}')
