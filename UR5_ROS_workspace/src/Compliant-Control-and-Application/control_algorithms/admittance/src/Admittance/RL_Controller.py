...
def main():
    ...
    date = now.strftime("M%mD%d-H%H-M%M")
    path = f'DATA_{date}/'
    os.mkdir(path)
    ...
    for ith_episode in range(num_episodes):
        ...
        while not controller.done:
            ...
            if len(rtau_buffer) >= n_step_update:
                ...
                # rt1 += -1*pow(np.linalg.norm(rtau_buffer),2)
        ...
        FNAME = path + f'DATA_episode{ith_episode}.npz'
        env.memory_save(FNAME, epi_reward=stats["epi_reward"],
                        epi_length=stats["epi_length"],
                        epi_energy=stats["epi_energy"],
                        epi_jerk=stats["epi_jerk"])
        # CLOSE EPISODE ===============================================
