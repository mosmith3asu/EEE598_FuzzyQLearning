...
class Admittance(object):
    ...
    def wait_for_transformations(self, reset=False):
        ...
        self.state['vel'] = 0#self.arm_desired_twist_adm_[0]
        self.state['acc'] = 0
        self.state['jerk'] = 0
    ...
    def compute_admittance(self):
        ...
        cmd_Twist1 = cmd_Twist0 + (Acc1 * dt)
        # cmd_Twist1 = obs_Twist0 + (Acc1 * dt)
    ...
    def state_wrench_callback(self, msg):
        ...
        for i in range(3):
            ...
            # lower force bound
            if abs(wrench_new[i]) < force_deadzone_lb:
                wrench_new[i] = 0
            elif wrench_new[i] > 0:
                wrench_new[i] -= force_deadzone_lb
            else:
                wrench_new[i] += force_deadzone_lb
            ...
            if abs(wrench_new[i]) > force_deadzone_ub:
                if wrench_new[i] > 0:
                    wrench_new[i] = force_deadzone_ub - force_deadzone_lb
                else:
                    wrench_new[i] = force_deadzone_ub + force_deadzone_lb
    ...
    def reset(self):
        ...
        while time.time() - tstart < timeout:
            ...
            idisp += 1
            if abs(dz) < move_tol and abs(self.current_linear_velocity[iz]) <move_tol: break
        self.arm_desired_twist_adm_ = np.zeros(6)
        self.send_command()
        ...
        while time.time() - tstart < timeout:
            ...
            idisp += 1
            if abs(dx) < move_tol and abs(self.current_linear_velocity[ix]) <move_tol: break
    ...
    def set_terminal(self, terminal_dict):
        ...
        self.terminal['max_duration'] = PUBLIC['max_epi_duration']#terminal_dict['max_duration']

    def check_terminal(self):  # Check at terminal state
        is_near_goal = (abs(self.state['pos'] - self.start_pos) > 0.15)
        within_var_tol = [(abs(self.state['vel']) < 0.01),
                          (abs(self.state['acc']) < 0.05),
        ...
        if is_near_goal:
            ...
            if is_terminal_state or exceeds_duration:
                print('\n\n########TERMINAL############\n\n')
                print(f'{is_terminal_state}')
                print(f'{exceeds_duration}')
                self.done = True
                self.report()
    ...
    def report(self):
        ...
        try:
            ...
            print(
                f'\nCalculate_admittance [{self.CONTROLLER_STATUS}] [N={len(self.hist["F"])}] [{round(self.state["timestamp"], 2)} {round(self.terminal["max_duration"],2)}]')
            print(
            ...
            print(f'\t| duration : {self.start_time} / {self.terminal["max_duration"]}')  # \t {np.round(np.max(self.hist["dt"]),5)}')
