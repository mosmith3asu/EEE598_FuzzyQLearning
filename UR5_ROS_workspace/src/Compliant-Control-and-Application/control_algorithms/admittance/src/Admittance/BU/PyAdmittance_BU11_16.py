...
class RT_lowpass_butter(object):
	def __init__(self,fs,fc,order,enable=True):
		self.enable = enable
		self.cutoff = fc  
		self.fs = fs
		self.order = order
		self.b, self.a = butter(order, fc, fs=fs, btype='low', analog=False)
		#self.a = 1
...
class Admittance(object):
	def __init__(self,dtau = None):
		...
		self.D = np.diag([5.,		D_const,D_const,D_const,D_const,D_const]) # Mat6x6d damping matrix
		...
		# Node -----------------------------------------------
		self.dt = 0.005 if dtau is None else dtau 			# controller sample freq/timestep
		rospy.init_node(self.node_handler, anonymous=True) 	# start ROS node
		self.loop_rate = rospy.Rate(1/self.dt)				# init ROS node freq
		...
		self.arm_max_damping_ = 20 	# maximum R action to change damping values
		self.arm_max_vel_ = 2.0		# maximum speed arm can travel
		self.arm_max_acc_ = 2.5		# maximum acc arm can achieve
	...
	def state_wrench_callback(self,msg): 
		...
		force_deadzone_lower_limit = 6.0
		force_deadzone_upper_limit = 30.0
		force_scale = 1.0 
...
if __name__=="__main__":
	terminal = {}
	terminal['pos'] = 0.2 # goal position
