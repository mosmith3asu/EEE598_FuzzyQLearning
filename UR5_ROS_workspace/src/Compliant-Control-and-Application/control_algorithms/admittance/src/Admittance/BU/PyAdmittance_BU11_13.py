...
class Admittance(object):
	def __init__(self):
		...
		M_gripper = 925 # 2-FINGER 85
		M_ee = M_gripper + M_FTsensor
		
		M_constrained = 5
		D_constrained = 50
		K_constrained = 100
		self.M = np.diag([0.1,	M_constrained,M_constrained,M_constrained,M_constrained,M_constrained])
		self.D = np.diag([1,	D_constrained,D_constrained,D_constrained,D_constrained,D_constrained])
	...
	def state_wrench_callback(self,msg):
		...
		force_deadzone_lower_limit = 30.0
		force_scale = 1.5
		filt_factor = 0.2
		...
		# Filter and update
		wrench_old = np.nan_to_num(self.current_ext_wrench_vec)
		
		ext_wrench = (1-filt_factor)*wrench_old + filt_factor*np.dot(rotation_ft_base, wrench_new.T)
		#print(rotation_ft_base)
		#print(wrench_new)
		#print(wrench_old)
		#print(ext_wrench)
		Fnorm = np.linalg.norm(ext_wrench) 
		if Fnorm < force_deadzone_lower_limit:
			ext_wrench[:3] = 0
		self.current_ext_wrench_vec = ext_wrench
